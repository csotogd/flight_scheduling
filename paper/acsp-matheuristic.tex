...
empirical. It is not a gradual degradation: on the set-I instance of each
of our families the base method closes gaps under 2\% up to nine thousand
shipments, and at GI scale needs a single branch-and-bound node to do so
(Section~\ref{sec:results:tree}); larger sets of the same families can sit
just above that line (MI at 4{,}000 shipments on its set-II instance
reaches 2.2\% in ten minutes). One family further up, the picture
...
& \sum_{p\in P_q\,:\,f\in p} y_p - d_q \sum_{s\,:\,f\in s} x_s \le 0,
  \qquad\quad\;\;\, \forall (q,f)\in \mathcal{C}\subseteq Q\times F^{O}
  \label{eq:ibc}
\end{align}
...
cuts of \cite{derigs2013}, separated on the fly for the violated pairs
$\mathcal{C}$ of shipments and \emph{own optional} flights. The restriction
to $F^{O}$ matters: for a mandatory flight the row is implied by
\eqref{eq:cover} together with the demand row \eqref{eq:demand} (the
flight is always covered, so the cut relaxes to
$\sum_{p} y_p \le d_q$), and for an external flight --- which no string covers ---
the right-hand side would vanish identically and the cut would wrongly
force flow off legally booked external capacity ($b_f=1$), cutting feasible
integer points. $P_q$ and $S$ are exponential and generated by
...
\Input{base instance $I_0$; batch size $B$; round cap $\bar{r}$; per-round
budget; eviction window $\bar{e}$; flat threshold $\theta$; flat-round
limit $\bar{f}$ (with zones, at least one full zone cycle)}
...
$(I^{\star},\sigma^{\star}) \leftarrow (I,\sigma)$;\quad
  $\text{flat} \leftarrow 0$;\quad $\text{empty} \leftarrow 0$\;
\For{$r=1,\dots,\bar{r}$ \KwSty{while} $\text{flat} < \bar{f}$}{
  ...
  \lIf{$C = \emptyset$}{\KwSty{stop} if $\mathord{+}\mathord{+}\text{empty}$
    completes a full zone cycle, \KwSty{else continue}}
  $\text{empty} \leftarrow 0$;\quad
  ...
  \leIf{$\mathrm{profit}(\sigma^{+})$ improves
    $\mathrm{profit}(\sigma^{\star})$ by less than $\theta$}{
    $\text{flat} \leftarrow \text{flat} + 1$}{$\text{flat} \leftarrow 0$}
  \lIf{$\mathrm{profit}(\sigma^{+}) > \mathrm{profit}(\sigma^{\star})$}{
    $(I^{\star},\sigma^{\star}) \leftarrow (I,\sigma^{+})$
    \tcp*{snapshot the instance $\sigma^{\star}$ flies on}}
  $\sigma \leftarrow \sigma^{+}$\;
}
\tcc{final solve: last instance plus rescued ever-flown evicted
candidates, full unconsolidated demand, seeded with the last schedule}
$\sigma_f \leftarrow \textsc{ExactFinal}\bigl(I \cup
  \{\text{evicted } c \text{ flown at least once}\},\ \sigma\bigr)$
  \tcp*{one fresh retry on failure}
\Return{$\sigma_f$ and its instance if the final solve succeeds (an exact
final replaces the coarse best), \KwSty{else} $(I^{\star},\sigma^{\star})$}
...
(ii) flows whose path traverses the \emph{kept set} on more than one maximal
run, and (iii) degenerate cuts (a run entering and leaving the region at the
same airport), whose flights are frozen because the merge splices exactly
one regional segment per donated flow. Class (ii) is deliberately stated on
the kept set rather than on geography, and recomputed inside the same
fixpoint as balance repair: repair removes intra-region flights from the
kept set while they keep operating, which can split a geographically single
crossing into two kept runs --- exactly the case the donation step must not
see.
...
frozen feeder lands plus transfer; a run starting at the shipment's true
origin is released at availability plus loading, and one ending at the true
destination is due at the deadline minus unloading. Cargo handling at true
origins and destinations is thereby encoded into the windows so gateway
hand-offs do not pay it twice. Consequently a time-feasible block solution
\emph{splices} back into a time-feasible global schedule by construction.
Shipments with both endpoints inside the region enter whole; under
deliver-all their contracted remainder rides along, so every block is
seeded with its share of the incumbent --- feasible for the block whenever
the learned fleet correction (below) has not shaved the slice beneath the
seed's own aircraft count.
...
reduction of $\max(\cdot, e_k)$ for the next visit. Two honest caveats.
The correction is deliberately \emph{not} clamped to the seed's own
aircraft count: with a fully used fleet the idle slack is zero and any
such clamp would permanently disable the learning --- the shave exists
precisely because the un-shaved budget produced infeasible merges. A
slice below the seed's count instead makes the block reject its own
starting point and run unseeded; that costs a warm start, never
feasibility (the merge guard). And the $\max$ update is a no-op when a
repeat failure overshoots by less than the current correction; the cycle
then simply keeps the incumbent for that block --- the rule is a
heuristic patch, not a convergence device.
...
$[\alpha,\beta]$, where $\alpha$ is the arrival of the frozen feeder plus
the gateway transfer time (the shipment's availability plus loading when
the run starts at its true origin) and $\beta$ the departure of the frozen
onward connection minus the transfer time (the deadline minus unloading
when the run ends at the true destination), all read from $\sigma$'s
unchanged timetable.
\end{proposition}

\begin{proof}
Frozen flights keep their departure and arrival times, so $\alpha$ and $\beta$
are unaffected by anything the block decides; at a true origin or
destination the corresponding endpoint is the shipment's own release or
deadline, which no block decision can move either. A block shipment's
release and deadline are set to exactly $\alpha$ and $\beta$ by
construction, and $\sigma_B$ respects them; splicing therefore preserves
every connection time on both sides of each gateway. It remains to
classify every incumbent flow. Flows of shipments with both endpoints in
the region are re-decided \emph{in full}: the merge drops them entirely
and installs the block's routing, so they never ride a half-touched path
(this covers, in particular, a whole-region shipment whose incumbent path
detoured outside the kept set). Every other flow either donates its single
kept run --- replaced under the window argument above --- or was frozen by
the partition step, which freezes every flow with more than one maximal
run through the kept set and every degenerate cut; because that criterion
is evaluated on the \emph{kept set} and re-applied within the same
fixpoint as balance repair, at the fixpoint each remaining flow has at
most one kept run and all its other legs are frozen. \qed
\end{proof}

One degenerate case is intentionally left to the runtime guard rather than
the construction: a donated run whose exact window closes to a
non-positive length is skipped (no sub-shipment is created) while its
flights remain re-decidable; a block that then drops them produces a merge
the independent check behind Proposition~\ref{prop:monotone} rejects.

\begin{proposition}[Monotonicity]\label{prop:monotone}
Started from an incumbent that is feasible for the original instance, the
regional cycle never decreases the global objective, and every schedule it
returns is feasible for the original instance.
\end{proposition}

\begin{proof}
Both statements are loop invariants. They hold at entry: the objective
trivially, feasibility by hypothesis (the caller hands the cycle a
feasibility-checked incumbent). Each iteration preserves them: a merged
schedule is adopted only if it passes the independent feasibility check
\emph{and} strictly improves the globally recomputed objective; otherwise
the incumbent is kept unchanged. They therefore hold of the returned
schedule in particular. \qed
\end{proof}
...
\begin{algorithm}[t]\small
\SetKwInOut{Input}{input}\SetKwInOut{Output}{output}
\Input{instance $I$, \emph{feasible} incumbent $\sigma$, regions
$\mathcal{R}$, per-block and total budgets}
...
\ForEach{cycle, \KwSty{and} pass $R$ over $\mathcal{R}$ then over
  ...
  \Repeat{$\mathcal{K}$ unchanged}{
    ...
    freeze the flights of flows with more than one maximal run through
      $\mathcal{K}$, and of degenerate cuts\;
  }
  ...
  $n^{B}_k \leftarrow n_k - \text{used}_k(\sigma) + \text{used}_k(\mathcal{K})
    - \text{shave}_k[R]$ \tcp*{slice; may reject the seed (see text)}
...
reproducible from the public repository: instances and every run output
are regenerated deterministically from their integer seeds by the
documented commands (raw logs themselves are not versioned; see the
Reproducibility section). One further disclosure: the campaign, including
Table~\ref{tab:tree}, was measured before the dual-bound certification
fix of Limitation~(viii), so its gaps are estimates until the certified
re-run.
...
\bottomrule
\end{tabular}
\caption{The base branch-and-price-and-cut across instance families (set-I
instances), short budgets, no matheuristic layer. Gaps were measured with
the pre-certification bound estimator (Limitation~(viii)).
$^{\dagger}$RLA is the deliver-all
...
escape, so pre-fix gaps --- including the 9.9\% above and every gap of
Table~\ref{tab:tree} --- are estimates, not certificates. The solver now
recomputes the Farley bound from an uncapped confirmation pass with a
flight-cover string aggregation and stamps every run with a certification
flag. A second audit pass (mid-September 2026) hardened the same account
further: an exhausted search tree no longer reports the incumbent as a
zero-gap certified bound when subtrees were pruned within the gap target
(the honest certificate is optimum $\le$ incumbent
$\cdot(1+\text{gap target})$); the maintenance string pricer's week window
was completed (a period-crossing predecessor could hide a feasible
connection two week-slots back, and an unconstrained elapsed-time limit
silently disabled string pricing altogether); and the rotation
feasibility checker now accepts the week-wrapping inter-string
connections the periodic master legally selects and prices. The
multi-seed battery of the road map will run entirely on the fixed
solver.
