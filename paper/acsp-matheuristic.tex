...
logs are available at the public repository accompanying this preprint.}}
...
4--5\,M of weekly profit in minutes of block time where the whole-network
...
Air cargo networks are planned in layers --- flight selection, fleet
assignment, rotation planning, cargo routing --- that interact strongly:
a flight is worth flying only if cargo can reach it, cargo can reach it only
if rotations position an aircraft, and an aircraft is available only if the
rest of the schedule releases it. Derigs and Friederichs
\cite{derigs2013} formulated the integrated problem (\acsp{}) over a cyclic
week, gave a path/string column formulation, and solved it by
branch-and-price-and-cut (B\&P\&C) with problem-specific branching and
implied-bound cuts, demonstrating the value of integration on four synthetic
airline archetypes with up to a few thousand shipments.

This paper asks an engineering question with methodological consequences:
\emph{what does it take to keep that integrated optimization useful one
order of magnitude higher}, at the scale of a global express integrator ---
tens of thousands of shipments per week, a thousand-plus flights, a
heterogeneous fleet --- and what parts of the exact machinery still earn
their keep there?
...
\item A measured engineering layer (Section~\ref{sec:engineering}):
  a bit-identical parallel pricing sweep, mispricing-safe dual smoothing
  \cite{wentges1997,dumerle1999}, Farley-type bounds \cite{farley1990} under
  truncation with a membership filter, and per-iteration cost breakdowns that
  make the next bottleneck visible instead of guessed.
...
arc-flow baseline are the declared next step.
...
root converges to the same LP value as the plain one.
...
Table~\ref{tab:waterfall} shows the round-0 waterfall on the consolidated
...
deliver-all). The last row is the ablation: without the ball, the same
...
${\sim}200$ stations --- motivates both the wave experiment (negative,
...
nominally designed for column-heavy LPs --- measured $2$--$3\times$
\emph{slower} per re-solve (49--114\,s) than warm-started dual simplex
(18--44\,s). Basis reuse across colgen iterations dominates the
specialized method.
